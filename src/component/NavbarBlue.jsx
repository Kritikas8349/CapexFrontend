...
>>>>>>> 8d65f6af33ebee1fe4d3c3d1fa122d84af977117
  ...
  const renderDropdown = (items) => (
    <ul>
      {items.map((item, i) => {
        ...
        // agar object hai (with subItems)
        return (
          <li key={i} className="has-submenu">
            {routesMap[item.title] ? (
              <Link
                to={routesMap[item.title]}
                onClick={(e) => e.stopPropagation()}
                className="nav-link"
              >
                {item.title}
    ...
    <ul className="overlay-submenu">
      {items.map((item, index) => {
        ...
        if (typeof item === "string") {
          return (
            <li key={key}>
              {routesMap[item] ? <Link to={routesMap[item]}>{item}</Link> : item}
            </li>
          );
        }
        return (
...
<<<<<<< HEAD
                onClick={(e) => { e.stopPropagation(); toggleSubMenu(menu); }}
=======
                onClick={(e) => {
                  e.stopPropagation();
                  toggleSubMenu(menu);
